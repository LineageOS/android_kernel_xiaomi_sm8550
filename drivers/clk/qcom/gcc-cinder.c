...
static struct clk_vdd_class *gcc_cinder_regulators[] = {
	&vdd_cx,
};
...
static struct clk_regmap *gcc_cinder_clocks[] = {
...
static const struct regmap_config gcc_cinder_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc gcc_cinder_desc = {
	.config = &gcc_cinder_regmap_config,
	.clks = gcc_cinder_clocks,
	.num_clks = ARRAY_SIZE(gcc_cinder_clocks),
	.resets = gcc_cinder_resets,
	.num_resets = ARRAY_SIZE(gcc_cinder_resets),
	.clk_regulators = gcc_cinder_regulators,
	.num_clk_regulators = ARRAY_SIZE(gcc_cinder_regulators),
};
...
static int gcc_cinder_probe(struct platform_device *pdev)
{
	...
	regmap = qcom_cc_map(pdev, &gcc_cinder_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &gcc_cinder_desc, regmap);
	if (ret) {
...
static void gcc_cinder_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_cinder_desc);
}

static struct platform_driver gcc_cinder_driver = {
	.probe = gcc_cinder_probe,
	.driver = {
		.name = "gcc-cinder",
		.of_match_table = gcc_cinder_match_table,
		.sync_state = gcc_cinder_sync_state,
	},
};

static int __init gcc_cinder_init(void)
{
	return platform_driver_register(&gcc_cinder_driver);
}
subsys_initcall(gcc_cinder_init);

static void __exit gcc_cinder_exit(void)
{
	platform_driver_unregister(&gcc_cinder_driver);
}
module_exit(gcc_cinder_exit);

MODULE_DESCRIPTION("QTI GCC CINDER Driver");
